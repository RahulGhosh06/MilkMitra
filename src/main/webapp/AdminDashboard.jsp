...
if(currentView == null || currentView.isEmpty())
{
    ...
    if(successMsg == null) successMsg = (String) request.getAttribute("successMsg");
    ...
    if("addFarmer".equals(currentView))       pageTitle = "Add Farmer";
    else if("farmerList".equals(currentView)) pageTitle = "View Farmers";
    else if("viewFarmer".equals(currentView)) pageTitle = "Farmer Profile";
    else if("editFarmer".equals(currentView)) pageTitle = "Edit Farmer";
    else if("payments".equals(currentView))   pageTitle = "Payments";
    else if("feedStore".equals(currentView))  pageTitle = "Feed Store";
    else if("reports".equals(currentView))    pageTitle = "Reports";
    else if("priceConfig".equals(currentView)) pageTitle = "Price Config";
%>
<!DOCTYPE html>
<html lang="en">
<head>
...
.content{padding:24px 32px 36px;flex:1;}
.bc{display:flex;align-items:center;gap:7px;font-size:12px;color:var(--muted);margin-bottom:20px;}
...
/* ═══════════════════════════════════════════════════════════
   ALERTS
═══════════════════════════════════════════════════════════ */
.alert{display:flex;align-items:center;gap:12px;padding:13px 16px;border-radius:10px;font-size:13px;font-weight:500;margin-bottom:20px;animation:slideDown .3s ease;}
...
.sc.blue::after{background:var(--blue);}   .sc.green::after{background:var(--green);}
.sc.amber::after{background:var(--amber);} .sc.red::after{background:var(--red);}
.sc.purple::after{background:var(--purple);} .sc.teal::after{background:var(--teal);}
.sc.orange::after{background:var(--orange);} .sc.cyan::after{background:#0891b2;}
...
.cc.teal::after{background:var(--teal);} .cc.blue::after{background:var(--blue);}
.cc.green::after{background:var(--green);} .cc.orange::after{background:var(--orange);}
...
.cc.teal .cc-head .si{background:var(--teal-lt);border:1px solid #99f6e4;color:var(--teal);}
.cc.blue .cc-head .si{background:var(--blue-lt);border:1px solid #bfdbfe;color:var(--blue);}
.cc.green .cc-head .si{background:var(--green-lt);border:1px solid #bbf7d0;color:var(--green);}
.cc.orange .cc-head .si{background:var(--orange-lt);border:1px solid #fed7aa;color:var(--orange);}
...
.cc-item .cv.green{color:var(--green);} .cc-item .cv.amber{color:var(--amber);}
.cc-item .cv.red{color:var(--red);}     .cc-item .cv.teal{color:var(--teal);}
.cc-item .cv.blue{color:var(--blue);}   .cc-item .cv.orange{color:var(--orange);}
...
/* ═══════════════════════════════════════════════════════════
   ADD FARMER VIEW
═══════════════════════════════════════════════════════════ */
.form-card{background:var(--white);border:1px solid var(--border);border-radius:var(--radius);overflow:hidden;margin-bottom:0;}
...
.field label{font-size:12px;font-weight:600;color:var(--navy);letter-spacing:.03em;}
...
.field input:focus,.field select:focus,.field textarea:focus{border-color:var(--blue);box-shadow:0 0 0 3px rgba(37,99,235,.10);background:var(--white);}
...
/* ═══════════════════════════════════════════════════════════
   FARMER LIST VIEW
═══════════════════════════════════════════════════════════ */
.toolbar{background:var(--white);border:1px solid var(--border);border-radius:var(--radius);padding:14px 16px;display:flex;align-items:center;gap:12px;margin-bottom:14px;}
...
.tw{background:var(--white);border-radius:var(--radius);border:1px solid var(--border);overflow:hidden;}
...
thead th{background:#f8faff;padding:11px 16px;text-align:left;font-size:10px;font-weight:700;color:var(--muted);text-transform:uppercase;letter-spacing:.6px;border-bottom:1px solid var(--border);}
...
.action-wrap{display:flex;align-items:center;gap:4px;flex-wrap:nowrap;}
.vbtn{display:inline-flex;align-items:center;justify-content:center;gap:4px;padding:6px 10px;border-radius:7px;font-size:11px;font-weight:600;text-decoration:none;transition:all .18s;cursor:pointer;white-space:nowrap;border:1px solid transparent;line-height:1;}
.vbtn i{font-size:13px;}
.vbtn-view{background:#eff6ff;color:#2563eb;border-color:#bfdbfe;width:30px;height:30px;padding:0;border-radius:7px;}
.vbtn-view:hover{background:#2563eb;color:#fff;border-color:#2563eb;box-shadow:0 2px 8px rgba(37,99,235,.3);transform:translateY(-1px);}
.vbtn-edit{background:#f0fdf4;color:#16a34a;border-color:#bbf7d0;width:30px;height:30px;padding:0;border-radius:7px;}
.vbtn-edit:hover{background:#16a34a;color:#fff;border-color:#16a34a;box-shadow:0 2px 8px rgba(22,163,74,.3);transform:translateY(-1px);}
.vbtn-deactivate{background:#fef2f2;color:#dc2626;border-color:#fecaca;padding:6px 10px;height:30px;}
.vbtn-deactivate:hover{background:#dc2626;color:#fff;border-color:#dc2626;box-shadow:0 2px 8px rgba(220,38,38,.25);transform:translateY(-1px);}
.vbtn-reactivate{background:#f0fdf4;color:#16a34a;border-color:#bbf7d0;padding:6px 10px;height:30px;}
.vbtn-reactivate:hover{background:#16a34a;color:#fff;border-color:#16a34a;box-shadow:0 2px 8px rgba(22,163,74,.25);transform:translateY(-1px);}
.vbtn-divider{width:1px;height:20px;background:var(--border);flex-shrink:0;margin:0 2px;}
...
.pb{width:30px;height:30px;display:flex;align-items:center;justify-content:center;border:1px solid var(--border);border-radius:6px;background:var(--white);color:var(--navy);font-size:12px;font-weight:500;cursor:pointer;}
...
═══════════════════════════════════════════════════════════ */
.profile-hero{background:linear-gradient(120deg,#0f2a6b,#1d4ed8);border-radius:var(--radius);padding:28px 32px;display:flex;align-items:center;gap:22px;margin-bottom:20px;position:relative;overflow:hidden;}
.profile-hero::after{content:'';position:absolute;right:-30px;top:-30px;width:160px;height:160px;background:rgba(255,255,255,.06);border-radius:50%;}
.ph-avatar{width:64px;height:64px;border-radius:50%;background:rgba(255,255,255,.18);border:2px solid rgba(255,255,255,.3);display:flex;align-items:center;justify-content:center;font-family:'DM Serif Display',serif;font-size:26px;color:#fff;flex-shrink:0;}
.ph-name{font-family:'DM Serif Display',serif;font-size:22px;color:#fff;margin-bottom:4px;}
.ph-meta{font-size:12px;color:#93c5fd;display:flex;align-items:center;gap:12px;flex-wrap:wrap;}
.ph-meta span{display:flex;align-items:center;gap:5px;}
.ph-status{display:inline-flex;align-items:center;gap:6px;padding:4px 14px;border-radius:20px;font-size:12px;font-weight:600;margin-top:10px;}
.ph-status.active{background:rgba(22,163,74,.22);color:#86efac;border:1px solid rgba(22,163,74,.3);}
.ph-status.inactive{background:rgba(220,38,38,.22);color:#fca5a5;border:1px solid rgba(220,38,38,.3);}
.detail-grid{display:grid;grid-template-columns:1fr 1fr;gap:14px;}
.detail-card{background:var(--white);border:1px solid var(--border);border-radius:var(--radius);overflow:hidden;}
.detail-card.full{grid-column:span 2;}
.dc-head{padding:12px 18px;border-bottom:1px solid var(--border);display:flex;align-items:center;gap:9px;background:#f8faff;}
.dc-head i{font-size:17px;color:var(--blue);}
.dc-head span{font-size:12px;font-weight:700;color:var(--navy);text-transform:uppercase;letter-spacing:.5px;}
.dc-body{padding:0;}
.dc-row{display:flex;align-items:baseline;padding:11px 18px;border-bottom:1px solid var(--border);}
.dc-row:last-child{border-bottom:none;}
.dc-lbl{font-size:11px;color:var(--muted);font-weight:600;width:160px;flex-shrink:0;}
.dc-val{font-size:13px;color:var(--navy);font-weight:500;}
.dc-val.code{color:var(--blue);font-weight:700;}
.profile-actions{display:flex;gap:12px;margin-top:18px;}
.pa-btn{display:inline-flex;align-items:center;gap:8px;padding:10px 22px;border-radius:9px;font-size:13px;font-weight:600;text-decoration:none;cursor:pointer;border:1.5px solid transparent;font-family:inherit;transition:all .18s;}
.pa-btn.edit{background:var(--navy);color:#fff;border-color:var(--navy);}
.pa-btn.edit:hover{background:#0d2a6b;}
.pa-btn.back{background:var(--white);color:var(--muted);border-color:var(--border);}
.pa-btn.back:hover{border-color:#b0bcd4;color:var(--text);}
.pa-btn.deactivate{background:var(--red-lt);color:var(--red);border-color:#fecaca;}
.pa-btn.deactivate:hover{background:var(--red);color:#fff;}
.pa-btn.reactivate{background:var(--green-lt);color:var(--green);border-color:#bbf7d0;}
.pa-btn.reactivate:hover{background:var(--green);color:#fff;}
.pa-btn i{font-size:16px;}
...
.ef-meta-line{font-size:12px;color:var(--muted);display:flex;align-items:center;gap:5px;}
.ef-meta-line i{font-size:14px;}
.ef-card{background:var(--white);border:1px solid var(--border);border-radius:var(--radius);overflow:hidden;margin-bottom:14px;}
.ef-sec-lbl{font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:.7px;color:var(--muted);padding:9px 20px 8px;background:#f8fafd;border-bottom:1px solid var(--border);display:flex;align-items:center;gap:7px;}
.ef-sec-lbl i{font-size:15px;}
.ef-fields{display:grid;grid-template-columns:1fr 1fr;}
.ef-field{padding:14px 20px;border-right:1px solid var(--border);border-bottom:1px solid var(--border);}
.ef-field:nth-child(2n){border-right:none;}
.ef-field.full{grid-column:1/-1;border-right:none;}
.ef-field label{display:flex;align-items:center;gap:5px;font-size:11px;font-weight:600;color:var(--muted);text-transform:uppercase;letter-spacing:.4px;margin-bottom:7px;}
.ef-field label i{font-size:13px;}
.ef-field input[type=text],.ef-field input[type=email],.ef-field textarea{width:100%;background:transparent;border:none;outline:none;font-size:14px;font-weight:600;color:var(--navy);font-family:'DM Sans',sans-serif;padding:0;resize:none;}
.ef-field input::placeholder,.ef-field textarea::placeholder{color:#c4cdd8;font-weight:400;}
.ef-field input[readonly]{color:var(--muted);}
.ro-wrap{display:flex;align-items:center;gap:7px;}
.ro-wrap i{font-size:13px;color:#c4cdd8;flex-shrink:0;}
.ef-actions{display:flex;gap:10px;padding-top:4px;}
...
.btn-back-ef{padding:12px 20px;background:var(--white);color:var(--muted);border:1px solid var(--border);border-radius:10px;font-size:14px;font-weight:600;font-family:'DM Sans',sans-serif;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:8px;text-decoration:none;transition:background .18s;}
.btn-back-ef:hover{background:var(--bg);}
.btn-back-ef i{font-size:17px;}
...
.modal-icon.red{background:var(--red-lt);color:var(--red);}
.modal-icon.green{background:var(--green-lt);color:var(--green);}
...
.btn-cancel{background:var(--bg);color:var(--navy);border:1px solid var(--border)!important;}
.btn-cancel:hover{border-color:var(--muted)!important;}
.btn-confirm-red{background:var(--red);color:#fff;}
.btn-confirm-red:hover{background:#b91c1c;}
.btn-confirm-green{background:var(--green);color:#fff;}
.btn-confirm-green:hover{background:#15803d;}

/* ═══════════════════════════════════════════════════════════
   WIP / UNDER CONSTRUCTION VIEW
═══════════════════════════════════════════════════════════ */
.wip-card{background:var(--white);border:1px solid var(--border);border-radius:var(--radius);overflow:hidden;}
.wip-hero{background:linear-gradient(120deg,#0f2a6b,#1d4ed8);padding:40px 32px;text-align:center;position:relative;overflow:hidden;}
.wip-hero::before{content:'';position:absolute;width:260px;height:260px;background:rgba(255,255,255,.06);border-radius:50%;top:-80px;right:-60px;}
.wip-hero::after{content:'';position:absolute;width:180px;height:180px;background:rgba(255,255,255,.04);border-radius:50%;bottom:-60px;left:-40px;}
.wip-icon-wrap{width:72px;height:72px;background:rgba(255,255,255,.15);border:2px solid rgba(255,255,255,.25);border-radius:20px;display:flex;align-items:center;justify-content:center;margin:0 auto 18px;position:relative;z-index:1;}
.wip-icon-wrap i{font-size:34px;color:#fff;}
.wip-hero h2{font-family:'DM Serif Display',serif;font-size:24px;color:#fff;margin-bottom:8px;position:relative;z-index:1;}
.wip-hero p{font-size:13px;color:#93c5fd;position:relative;z-index:1;}
.wip-body{padding:28px 32px;}
.wip-status-row{display:flex;align-items:center;justify-content:center;gap:10px;margin-bottom:24px;}
.wip-badge{display:inline-flex;align-items:center;gap:7px;padding:7px 16px;border-radius:20px;font-size:12px;font-weight:600;}
.wip-badge.building{background:#fffbeb;color:#92400e;border:1px solid #fde68a;}
.wip-badge.planned{background:var(--blue-lt);color:#1e40af;border:1px solid #bfdbfe;}
.wip-badge i{font-size:14px;}
.wip-features{margin-bottom:24px;}
.wip-features h4{font-size:12px;font-weight:700;color:var(--muted);text-transform:uppercase;letter-spacing:.6px;margin-bottom:12px;}
.feat-chips{display:flex;flex-wrap:wrap;gap:8px;}
.feat-chip{display:inline-flex;align-items:center;gap:6px;padding:6px 14px;background:var(--bg);border:1px solid var(--border);border-radius:8px;font-size:12px;color:var(--navy);font-weight:500;}
.feat-chip i{font-size:14px;color:var(--muted);}
.wip-back{display:inline-flex;align-items:center;gap:8px;padding:10px 22px;background:var(--navy);color:#fff;border-radius:9px;text-decoration:none;font-size:13px;font-weight:600;transition:background .18s;}
.wip-back:hover{background:#0d2a6b;}
.wip-back i{font-size:16px;}
</style>
</head>
<body>
...
<aside class="sidebar">
    ...
    <nav class="sb-nav">
        ...
        <a href="AdminDashboardServlet?view=dashboard" class="ni <%= "dashboard".equals(currentView) ? "active" : "" %>">
            <i class="ti ti-home"></i>Dashboard
        </a>

        <div class="nl">Farmer Management</div>
        <a href="AdminDashboardServlet?view=addFarmer" class="ni <%= "addFarmer".equals(currentView) ? "active" : "" %>">
            <i class="ti ti-user-plus"></i>Add Farmer
        </a>
        <a href="FarmerListServlet?view=farmerList" class="ni <%= "farmerList".equals(currentView)||"viewFarmer".equals(currentView)||"editFarmer".equals(currentView) ? "active" : "" %>">
        ...
        <a href="MilkCollectionReportServlet?view=today" class="ni"><i class="ti ti-droplet-filled-2"></i>Collection Report</a>
        <a href="AdminDashboardServlet?view=payments" class="ni <%= "payments".equals(currentView) ? "active" : "" %>"><i class="ti ti-cash"></i>Payments</a>
        <a href="AdminDashboardServlet?view=feedStore" class="ni <%= "feedStore".equals(currentView) ? "active" : "" %>"><i class="ti ti-wheat"></i>Feed Store</a>

        <div class="nl">Reports & Settings</div>
        <a href="AdminDashboardServlet?view=reports" class="ni <%= "reports".equals(currentView) ? "active" : "" %>"><i class="ti ti-chart-bar"></i>Reports</a>
        <a href="AdminDashboardServlet?view=priceConfig" class="ni <%= "priceConfig".equals(currentView) ? "active" : "" %>"><i class="ti ti-currency-rupee"></i>Price Config</a>
    </nav>
    <div class="sb-foot">
...
<div class="main">
    ...
    <div class="topbar">
        <div class="tb-l">
            ...
            <p id="topbarSub">
                <% if("dashboard".equals(currentView)) { %>Overview of your dairy operations
                <% } else if("addFarmer".equals(currentView)) { %>Register a new farmer to MilkMitra
                <% } else if("farmerList".equals(currentView)) { %>Manage and monitor registered farmers
                <% } else if("viewFarmer".equals(currentView) && farmer != null) { %>Viewing profile of <%= farmer.getFarmerName() %>
                <% } else if("editFarmer".equals(currentView) && farmer != null) { %>Editing profile of <%= farmer.getFarmerName() %>
                <% } else if("payments".equals(currentView)) { %>10-day cycle payment management
                <% } else if("feedStore".equals(currentView)) { %>Feed orders and installments
                <% } else if("reports".equals(currentView)) { %>Analytics and export reports
                <% } else if("priceConfig".equals(currentView)) { %>Configure milk rate formulas
                <% } %>
            </p>
        </div>
        <div class="tb-r">
    ...
    <div class="content">
        ...
        <!-- Today's Collection -->
        <div class="sec-head">
            <div class="sec-title"><i class="ti ti-calendar-today"></i>Today's Collection</div>
        ...
        <div class="grid-2">
            <div class="cc teal">
                ...
                <div class="cc-grid">
                    <div class="cc-item"><div class="cl">☀️ Morning</div><div class="cv teal"><%= morningEntries %></div></div>
                    <div class="cc-item"><div class="cl">🌙 Evening</div><div class="cv blue"><%= eveningEntries %></div></div>
                </div>
            </div>

            <div class="cc blue">
                <div class="cc-head">
                    <div class="si blue"><i class="ti ti-droplet"></i></div>
                    <div>
                ...
                <div class="cc-grid">
                    <div class="cc-item"><div class="cl">🐄 Cow Milk</div><div class="cv green"><%= String.format("%.2f",todayCowLtr) %> L</div></div>
                    <div class="cc-item"><div class="cl">🐃 Buffalo Milk</div><div class="cv amber"><%= String.format("%.2f",todayBufLtr) %> L</div></div>
                </div>
            </div>
        </div>

        <div class="grid-4">
            <div class="sc green"><div class="si green"><i class="ti ti-coin-rupee"></i></div><div class="sc-body"><div class="val">₹<%= String.format("%.2f",todayValue) %></div><div class="lbl">Today's Value</div><div class="sub">Est. payout</div></div></div>
            <div class="sc teal"><div class="si teal"><i class="ti ti-chart-dots"></i></div><div class="sc-body"><div class="val"><%= String.format("%.2f",avgFat) %>%</div><div class="lbl">Average FAT</div><div class="sub">Today's average</div></div></div>
            <div class="sc blue"><div class="si blue"><i class="ti ti-chart-dots-2"></i></div><div class="sc-body"><div class="val"><%= String.format("%.2f",avgSnf) %>%</div><div class="lbl">Average SNF</div><div class="sub">Today's average</div></div></div>
            <div class="sc purple"><div class="si purple"><i class="ti ti-wheat"></i></div><div class="sc-body"><div class="val"><%= pendingFeedOrders %></div><div class="lbl">Feed Orders</div><div class="sub">Pending delivery</div></div></div>
        </div>
        ...
        <div class="absent-card" style="margin-bottom:24px;">
            ...
            <div>
                ...
                <div class="ab-sub">Active farmers who gave milk yesterday but have not collected today yet.<% if(absentToday==0){%> ✅ All active farmers have collected today!<%}%></div>
            </div>
        </div>
        ...
        <!-- Farmer Overview -->
        <div class="sec-head">
            <div class="sec-title"><i class="ti ti-users"></i>Farmer Overview</div>
        </div>
        <div class="grid-4">
            <div class="sc blue"><div class="si blue"><i class="ti ti-users"></i></div><div class="sc-body"><div class="val"><%= totalFarmers %></div><div class="lbl">Total Farmers</div><div class="sub">All registered</div></div></div>
            <div class="sc green"><div class="si green"><i class="ti ti-user-check"></i></div><div class="sc-body"><div class="val"><%= activeFarmers %></div><div class="lbl">Active Farmers</div><div class="sub">Currently supplying</div></div></div>
            <div class="sc amber"><div class="si amber"><i class="ti ti-cash"></i></div><div class="sc-body"><div class="val">₹<%= String.format("%.0f",pendingPayments) %></div><div class="lbl">Pending Payments</div><div class="sub">Awaiting release</div></div></div>
            <div class="sc red"><div class="si red"><i class="ti ti-alert-triangle"></i></div><div class="sc-body"><div class="val"><%= lowStockItems %></div><div class="lbl">Low Stock Alerts</div><div class="sub">Inventory items</div></div></div>
        </div>
        ...
        <!-- Quick Access -->
        <div class="sec-head">
            <div class="sec-title"><i class="ti ti-layout-grid"></i>Quick Access</div>
        </div>
        <div class="modules">
            <a href="FarmerListServlet?view=farmerList" class="mod-card" style="--cc:#2563eb;">
                <div class="mc-icon blue"><i class="ti ti-users"></i></div>
                <div class="mc-title">Manage Farmers</div><div class="mc-sub">View · Edit · Deactivate</div>
                <div class="mc-arrow">Open →</div>
            </a>
            <a href="AdminDashboardServlet?view=addFarmer" class="mod-card" style="--cc:#16a34a;">
                <div class="mc-icon green"><i class="ti ti-user-plus"></i></div>
                <div class="mc-title">Add Farmer</div><div class="mc-sub">Register new farmer</div>
                <div class="mc-arrow">Open →</div>
            </a>
            <a href="MilkCollection.jsp" class="mod-card" style="--cc:#0d9488;">
                <div class="mc-icon teal"><i class="ti ti-droplet"></i></div>
                <div class="mc-title">Milk Collection</div><div class="mc-sub">Morning &amp; evening entry</div>
                <div class="mc-arrow">Open →</div>
            </a>
            <a href="AdminDashboardServlet?view=payments" class="mod-card" style="--cc:#d97706;">
                <div class="mc-icon amber"><i class="ti ti-cash"></i></div>
                <div class="mc-title">Payments</div><div class="mc-sub">10-day cycle payments</div>
                <div class="mc-arrow">Open →</div>
            </a>
            <a href="AdminDashboardServlet?view=feedStore" class="mod-card" style="--cc:#7c3aed;">
                <div class="mc-icon purple"><i class="ti ti-wheat"></i></div>
                <div class="mc-title">Feed Store</div><div class="mc-sub">Orders &amp; installments</div>
                <div class="mc-arrow">Open →</div>
            </a>
            <a href="AdminDashboardServlet?view=priceConfig" class="mod-card" style="--cc:#dc2626;">
                <div class="mc-icon red"><i class="ti ti-currency-rupee"></i></div>
                <div class="mc-title">Price Config</div><div class="mc-sub">Update rate formulas</div>
                <div class="mc-arrow">Open →</div>
            </a>
            <a href="AdminDashboardServlet?view=reports" class="mod-card" style="--cc:#0891b2;">
                <div class="mc-icon cyan"><i class="ti ti-chart-bar"></i></div>
                <div class="mc-title">Reports</div><div class="mc-sub">Analytics &amp; exports</div>
                <div class="mc-arrow">Open →</div>
            </a>
        </div>
        ...
        <!-- Breadcrumb -->
        <div class="bc">
            <a href="AdminDashboardServlet?view=dashboard">Dashboard</a>
        ...
        <div class="form-card">
                ...
                <div class="form-section">
                    ...
                    <div class="form-grid">
                        ...
                        <div class="field">
                            ...
                            <input type="text" name="ifscCode" id="ifscInput" placeholder="e.g. SBIN0001234" style="text-transform:uppercase" maxlength="11" required/>
                        </div>
                        <div class="field">
            ...
            <div class="form-footer">
                ...
                <div class="btn-group">
                    <a href="AdminDashboardServlet?view=dashboard" class="btn btn-outline"><i class="ti ti-x"></i>Cancel</a>
...
<% } %>
        ...
        <div class="grid-4" style="margin-bottom:14px;">
            <div class="sc blue"><div class="si blue"><i class="ti ti-users"></i></div><div class="sc-body"><div class="val"><%= farmerTotal %></div><div class="lbl">All Farmers</div></div></div>
            <div class="sc green"><div class="si green"><i class="ti ti-user-check"></i></div><div class="sc-body"><div class="val"><%= activeCount %></div><div class="lbl">Active Farmers</div></div></div>
            <div class="sc red"><div class="si red"><i class="ti ti-user-x"></i></div><div class="sc-body"><div class="val"><%= inactiveCount %></div><div class="lbl">Inactive Farmers</div></div></div>
            <div class="sc amber"><div class="si amber"><i class="ti ti-calendar-stats"></i></div><div class="sc-body"><div class="val"><%= joinedThisMonth %></div><div class="lbl">Joined This Month</div></div></div>
        </div>
        ...
        <div class="toolbar">
            <div class="srch-wrap">
                ...
                <input type="text" id="srch" placeholder="Search by Code / Name / Mobile..." oninput="doFilter()">
            </div>
            ...
            <a href="FarmerListServlet?view=farmerList" class="ref-btn"><i class="ti ti-refresh"></i>Refresh</a>
        </div>
        ...
        <div class="tw">
            <div class="tw-head">
                ...
                <span>Showing <span id="sc2"><%= farmerTotal %></span> of <%= farmerTotal %> farmers</span>
            </div>
            <table>
                <thead>
                    <tr>
                ...
                %>
                    <tr data-code="<%= f.getFarmerCode().toLowerCase() %>"
                        ...
                        <td>
                            <% if(f.isActive()) { %>
                                <span class="status-badge active"><i class="ti ti-circle-filled"></i>Active</span>
                            <% } else { %>
                                <span class="status-badge inactive"><i class="ti ti-circle-filled"></i>Inactive</span>
                            <% } %>
                        </td>
                        <td>
                           <div class="action-wrap">
                                <a href="ViewFarmerServlet?farmerCode=<%= f.getFarmerCode() %>"
                                    class="vbtn vbtn-view" title="View Profile">
                                    <i class="ti ti-eye"></i>
                                </a>
                                <a href="LoadEditFarmerServlet?farmerCode=<%= f.getFarmerCode() %>"
                                    class="vbtn vbtn-edit" title="Edit Farmer">
                ...
                <% } } else { %>
                    <tr><td colspan="7">
                        <div class="empty">
                            ...
                            <p>No farmers found.
                               <a href="AdminDashboardServlet?view=addFarmer" style="color:var(--blue);">Add a farmer</a> to get started.
                            </p>
                        </div>
                    </td></tr>
                <% } %>
            ...
            <div class="pag">
                <span class="pi" id="pi">Showing 1 to <%= farmerTotal %> of <%= farmerTotal %> farmers</span>
                <div class="pbtns">
        ...
        <% } else if("viewFarmer".equals(currentView)) {
               if(farmer == null) { %>
               <div class="alert alert-error"><i class="ti ti-alert-circle"></i>Farmer not found. <a href="AdminDashboardServlet?view=farmerList">Go back to list</a>.</div>
        <% } else { %>

        <!-- Breadcrumb -->
        <div class="bc">
            <a href="FarmerListServlet?view=farmerList">View Farmers</a>
            <i class="ti ti-chevron-right"></i>
            <a href="AdminDashboardServlet?view=farmerList">View Farmers</a>
            <i class="ti ti-chevron-right"></i>
            <span><%= farmer.getFarmerName() %></span>
        </div>

        <!-- Hero -->
        <div class="profile-hero">
            <div class="ph-avatar"><%= farmer.getFarmerName() != null && farmer.getFarmerName().length() > 0 ? farmer.getFarmerName().substring(0,1).toUpperCase() : "F" %></div>
            <div>
                ...
                <div class="ph-status <%= farmer.isActive() ? "active" : "inactive" %>">
        ...
        <div class="detail-grid">
            ...
            <div class="detail-card">
                <div class="dc-head"><i class="ti ti-user"></i><span>Personal Information</span></div>
                <div class="dc-body">
                    <div class="dc-row"><span class="dc-lbl">Farmer Code</span><span class="dc-val code"><%= farmer.getFarmerCode() %></span></div>
                    <div class="dc-row"><span class="dc-lbl">Full Name</span><span class="dc-val"><%= farmer.getFarmerName() %></span></div>
                    <div class="dc-row"><span class="dc-lbl">Mobile</span><span class="dc-val">+91 <%= farmer.getMobile() %></span></div>
                    <div class="dc-row"><span class="dc-lbl">Email</span><span class="dc-val"><%= (farmer.getEmail() != null && !farmer.getEmail().isEmpty()) ? farmer.getEmail() : "—" %></span></div>
                    <div class="dc-row"><span class="dc-lbl">Aadhaar No.</span><span class="dc-val"><%= (farmer.getAndharNo() != null) ? farmer.getAndharNo() : "—" %></span></div>
                    <% if(farmer.getJoiningDate() != null) { %>
                    <div class="dc-row"><span class="dc-lbl">Joining Date</span><span class="dc-val"><%= farmer.getJoiningDate() %></span></div>
                    <% } %>
                </div>
            </div>
            ...
            <!-- Bank -->
            <div class="detail-card">
                <div class="dc-head"><i class="ti ti-building-bank"></i><span>Bank Details</span></div>
                <div class="dc-body">
                    <div class="dc-row"><span class="dc-lbl">Account Holder</span><span class="dc-val"><%= (farmer.getAccountHolderName() != null) ? farmer.getAccountHolderName() : "—" %></span></div>
                    <div class="dc-row"><span class="dc-lbl">Account Number</span><span class="dc-val"><%= (farmer.getAccountNo() != null) ? farmer.getAccountNo() : "—" %></span></div>
                    <div class="dc-row"><span class="dc-lbl">Bank Name</span><span class="dc-val"><%= (farmer.getBankName() != null) ? farmer.getBankName() : "—" %></span></div>
                    <div class="dc-row"><span class="dc-lbl">IFSC Code</span><span class="dc-val"><%= (farmer.getIfscCode() != null) ? farmer.getIfscCode() : "—" %></span></div>
                </div>
            </div>

            <!-- Address -->
            <div class="detail-card full">
                <div class="dc-head"><i class="ti ti-map-pin"></i><span>Address</span></div>
                <div class="dc-body">
                    <div class="dc-row"><span class="dc-lbl">Full Address</span><span class="dc-val"><%= (farmer.getAddress() != null && !farmer.getAddress().isEmpty()) ? farmer.getAddress() : "—" %></span></div>
                </div>
            </div>
        </div>
        ...
        <div class="profile-actions">
            <a href="AdminDashboardServlet?view=farmerList" class="pa-btn back"><i class="ti ti-arrow-left"></i>Back to List</a>
            <a href="LoadEditFarmerServlet?farmerCode=<%= farmer.getFarmerCode() %>" class="pa-btn edit"><i class="ti ti-pencil"></i>Edit Farmer</a>
            <% if(farmer.isActive()) { %>
        ...
        <div class="bc">
            ...
            <a href="AdminDashboardServlet?view=farmerList">View Farmers</a>
            <i class="ti ti-chevron-right"></i>
            <span>Edit · <%= farmer.getFarmerName() %></span>
        </div>
        ...
        <div class="ef-header">
            ...
            <div>
                ...
                <p class="ef-meta-line"><i class="ti ti-hash"></i><%= farmer.getFarmerCode() %> &nbsp;·&nbsp; Edit farmer profile</p>
            </div>
        </div>

        <form action="EditFarmerServlet" method="post">
            <input type="hidden" name="farmerCode" value="<%= farmer.getFarmerCode() %>">
            <input type="hidden" name="returnTo" value="AdminDashboard">

            <!-- Personal -->
            <div class="ef-card">
                <div class="ef-sec-lbl"><i class="ti ti-user"></i>Personal information</div>
                <div class="ef-fields">
                    <div class="ef-field">
                    ...
                    <div class="ef-field">
                        ...
                        <input type="email" name="email" value="<%= farmer.getEmail() != null ? farmer.getEmail() : "" %>" placeholder="email@example.com">
                    </div>
            ...
            <!-- Bank -->
            <div class="ef-card">
                <div class="ef-sec-lbl"><i class="ti ti-building-bank"></i>Bank details</div>
                <div class="ef-fields">
                    <div class="ef-field">
                        <label><i class="ti ti-user-circle"></i>Account holder name</label>
                        <input type="text" name="accountHolderName" value="<%= farmer.getAccountHolderName() != null ? farmer.getAccountHolderName() : "" %>" placeholder="As per bank records">
                    </div>
                    <div class="ef-field">
                        <label><i class="ti ti-credit-card"></i>Account number</label>
                        <input type="text" name="bankAccountNo" value="<%= farmer.getAccountNo() != null ? farmer.getAccountNo() : "" %>" placeholder="Account number">
                    </div>
                    <div class="ef-field">
                        <label><i class="ti ti-building"></i>Bank name</label>
                        <input type="text" name="bankName" value="<%= farmer.getBankName() != null ? farmer.getBankName() : "" %>" placeholder="Bank name">
                    </div>
                    <div class="ef-field">
                        <label><i class="ti ti-code"></i>IFSC code</label>
                        <input type="text" name="ifscCode" value="<%= farmer.getIfscCode() != null ? farmer.getIfscCode() : "" %>" placeholder="e.g. SBIN0001234">
                    </div>
                </div>
            </div>
            ...
            <!-- Identity -->
            <div class="ef-card">
                <div class="ef-sec-lbl"><i class="ti ti-id-badge"></i>Identity</div>
                <div class="ef-fields">
                    <div class="ef-field full">
                        <label><i class="ti ti-fingerprint"></i>Aadhaar number</label>
                        <input type="text" name="andharNo" value="<%= farmer.getAndharNo() != null ? farmer.getAndharNo() : "" %>" placeholder="12-digit Aadhaar">
                    </div>
                </div>
            </div>

            <div class="ef-actions">
                <a href="AdminDashboardServlet?view=farmerList" class="btn-back-ef"><i class="ti ti-arrow-left"></i>Back</a>
        ...
        <div class="wip-card">
            <div class="wip-hero">
                <div class="wip-icon-wrap"><i class="ti ti-cash"></i></div>
                <h2>Payments Module</h2>
                <p>10-day cycle payment management for all registered farmers</p>
            </div>
            <div class="wip-body">
                <div class="wip-status-row">
                    <span class="wip-badge building"><i class="ti ti-tools"></i>Under Development</span>
                    <span class="wip-badge planned"><i class="ti ti-clock"></i>Coming Soon</span>
                </div>
                <div class="wip-features">
                    <h4>Planned Features</h4>
                    <div class="feat-chips">
                        <span class="feat-chip"><i class="ti ti-calendar-repeat"></i>10-day payment cycles</span>
                        <span class="feat-chip"><i class="ti ti-receipt"></i>Payment vouchers</span>
                        <span class="feat-chip"><i class="ti ti-building-bank"></i>Bank transfer records</span>
                        <span class="feat-chip"><i class="ti ti-file-invoice"></i>Payment history</span>
                        <span class="feat-chip"><i class="ti ti-coin-rupee"></i>Advance deductions</span>
                        <span class="feat-chip"><i class="ti ti-printer"></i>Printable statements</span>
                    </div>
                </div>
                <a href="AdminDashboardServlet?view=dashboard" class="wip-back"><i class="ti ti-arrow-left"></i>Back to Dashboard</a>
            </div>
        </div>
        ...
        <div class="bc">
            <a href="AdminDashboardServlet?view=dashboard">Dashboard</a>
            <i class="ti ti-chevron-right"></i>
            <span>Feed Store</span>
        </div>

        <div class="wip-card">
            <div class="wip-hero">
                <div class="wip-icon-wrap"><i class="ti ti-wheat"></i></div>
            ...
            <div class="wip-body">
                ...
                <div class="wip-features">
                    <h4>Planned Features</h4>
                    <div class="feat-chips">
                        <span class="feat-chip"><i class="ti ti-package"></i>Feed inventory</span>
                        <span class="feat-chip"><i class="ti ti-shopping-cart"></i>Order management</span>
                        <span class="feat-chip"><i class="ti ti-credit-card"></i>Installment tracking</span>
                        <span class="feat-chip"><i class="ti ti-alert-triangle"></i>Low stock alerts</span>
                        <span class="feat-chip"><i class="ti ti-truck"></i>Delivery tracking</span>
                        <span class="feat-chip"><i class="ti ti-chart-bar"></i>Consumption reports</span>
                    </div>
                </div>
                <a href="AdminDashboardServlet?view=dashboard" class="wip-back"><i class="ti ti-arrow-left"></i>Back to Dashboard</a>
            </div>
        ...
        <div class="wip-card">
            <div class="wip-hero">
                <div class="wip-icon-wrap"><i class="ti ti-chart-bar"></i></div>
            ...
            <div class="wip-body">
                ...
                <div class="wip-features">
                    <h4>Planned Features</h4>
                    <div class="feat-chips">
                        <span class="feat-chip"><i class="ti ti-calendar-stats"></i>Monthly summaries</span>
                        <span class="feat-chip"><i class="ti ti-users"></i>Farmer-wise reports</span>
                        <span class="feat-chip"><i class="ti ti-chart-line"></i>Trend analysis</span>
                        <span class="feat-chip"><i class="ti ti-file-export"></i>Excel &amp; PDF export</span>
                        <span class="feat-chip"><i class="ti ti-droplet"></i>Milk quality reports</span>
                        <span class="feat-chip"><i class="ti ti-coin-rupee"></i>Financial summaries</span>
                    </div>
                </div>
                <a href="AdminDashboardServlet?view=dashboard" class="wip-back"><i class="ti ti-arrow-left"></i>Back to Dashboard</a>
            </div>
        ...
        <div class="bc">
            <a href="AdminDashboardServlet?view=dashboard">Dashboard</a>
            <i class="ti ti-chevron-right"></i>
            <span>Price Config</span>
        </div>

        <div class="wip-card">
            <div class="wip-hero">
                <div class="wip-icon-wrap"><i class="ti ti-currency-rupee"></i></div>
                <h2>Price Configuration</h2>
                <p>Configure milk rate formulas, FAT/SNF slabs and pricing rules</p>
            </div>
            <div class="wip-body">
                <div class="wip-status-row">
                    <span class="wip-badge building"><i class="ti ti-tools"></i>Under Development</span>
                ...
                <div class="wip-features">
                    <h4>Planned Features</h4>
                    <div class="feat-chips">
                        <span class="feat-chip"><i class="ti ti-math"></i>FAT/SNF rate formula</span>
                        <span class="feat-chip"><i class="ti ti-table"></i>Price slab configuration</span>
                        <span class="feat-chip"><i class="ti ti-toggle-right"></i>Cow vs Buffalo rates</span>
                        <span class="feat-chip"><i class="ti ti-history"></i>Rate change history</span>
                        <span class="feat-chip"><i class="ti ti-calculator"></i>Rate preview calculator</span>
                        <span class="feat-chip"><i class="ti ti-calendar"></i>Effective date setting</span>
                    </div>
                </div>
                <a href="AdminDashboardServlet?view=dashboard" class="wip-back"><i class="ti ti-arrow-left"></i>Back to Dashboard</a>
            </div>
...
<div class="modal-overlay" id="deactivateModal">
    <div class="modal">
        <div class="modal-icon red"><i class="ti ti-user-off"></i></div>
        <h3>Deactivate Farmer?</h3>
        <p>You are about to deactivate<br><strong id="deactivateName">—</strong> (<span id="deactivateCode">—</span>).<br>They will no longer be able to supply milk.</p>
        <div class="modal-btns">
            <button class="btn-cancel" onclick="closeModal('deactivateModal')">Cancel</button>
            <a id="deactivateConfirmBtn" href="#" class="btn-confirm-red">Yes, Deactivate</a>
        </div>
    </div>
</div>
...
<div class="modal-overlay" id="reactivateModal">
    <div class="modal">
        <div class="modal-icon green"><i class="ti ti-user-check"></i></div>
        <h3>Reactivate Farmer?</h3>
        <p>You are about to reactivate<br><strong id="reactivateName">—</strong> (<span id="reactivateCode">—</span>).<br>They will be able to supply milk again.</p>
        <div class="modal-btns">
            <button class="btn-cancel" onclick="closeModal('reactivateModal')">Cancel</button>
            <a id="reactivateConfirmBtn" href="#" class="btn-confirm-green">Yes, Reactivate</a>
        </div>
    </div>
</div>
...
function doFilter(){
    var search = document.getElementById('srch').value.toLowerCase().trim();
    ...
    rows.forEach(function(row,idx){
        ...
        var show = true;
        if(search && !code.includes(search) && !name.includes(search) && !mob.includes(search)) show=false;
        ...
        if(filter==='thismonth' && date){
            ...
            if(joined.getMonth()!==today.getMonth()||joined.getFullYear()!==today.getFullYear()) show=false;
        }
        row.style.display = show?'':'none';
        if(show){ visible++; row.querySelector('td:first-child').textContent=visible; }
    });
    var sc2=document.getElementById('sc2');
    var pi=document.getElementById('pi');
    if(sc2) sc2.textContent=visible;
    if(pi)  pi.textContent='Showing 1 to '+visible+' of '+visible+' farmers';
}
...
function showModal(type, code, name){
    if(type==='deactivate'){
        ...
        document.getElementById('deactivateConfirmBtn').href  =
            'DeactivateFarmerServlet?farmerCode='+code+'&returnTo=AdminDashboard';
        ...
        document.getElementById('reactivateConfirmBtn').href  =
            'ReactivateFarmerServlet?farmerCode='+code+'&returnTo=AdminDashboard';
